<div>
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Création d'une publication</h1>
        <nav class="flex items-center text-sm text-gray-400 space-x-2" aria-label="Breadcrumb">
            ...
            <span>/</span>
            <a href="{{ route('admin.posts') }}" class="hover:text-white">Publications</a>
            <span>/</span>
        ...
        <form wire:submit='store'>
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            wire:model.blur="form.category_post_id">
                            <option value="">Selectionner une category</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        @error('form.category_post_id')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <div wire:ignore x-data x-init="ClassicEditor
                        .create($refs.editor)
                        .then(editor => {
                            ...
                            window.addEventListener('reset-editor', () => {
                                editor.setData(@this.get('form.content') || '');
                    
                            });
                        })
                        .catch(error => { console.error(error); });">
                        <textarea x-ref="editor" id="content"
                            class=" hidden w-full rounded-lg border border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 focus:outline-none py-2 px-4 text-gray-900 placeholder-gray-400 transition resize-none h-32"
                            placeholder="Décrivez la formation ici...">{{ $form->content }}</textarea>
                    </div>
                ...
                <div class="flex justify-end space-x-3">
                    <a href="{{ route('admin.posts') }}" id="cancelFormationBtn" class="btn-secondary">Annuler</a>
